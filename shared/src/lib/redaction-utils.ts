 ...
 */
export const DEFAULT_PII_PATTERNS: PIIField[] = [
...
export function redactString(
  ...
  if (matchingField) {
    return applyRedactionMethod(
      value,
      matchingField.method,
      matchingField.replacement
    );
  }
 ...
 */
function maskString(value: string, mask?: string): string {
  if (!mask) {
...
export function isPIIField(
  fieldName: string,
  patterns: PIIField[] = DEFAULT_PII_PATTERNS
): boolean {
 ...
 */
export function createSafeLogObject(
  obj: unknown,
  config: RedactionConfig = DEFAULT_REDACTION_CONFIG
): unknown {
