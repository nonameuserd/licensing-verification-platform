...
class PrivacyLogger {
    constructor(config) {
        ...
        this.auditLogger = (0, pino_1.default)({
            ...
            formatters: {
                level: (label) => ({ level: label }),
                log: (object) => (0, redaction_utils_1.redactObject)(object, this.redactionConfig),
            },
            base: {
                service: this.config.serviceName,
                environment: this.config.environment,
    ...
    child(context) {
        const childConfig = {
            ...this.config,
            serviceName: `${this.config.serviceName}-${context['service'] || 'child'}`,
        };
...
function createPrivacyLogger(config) {
    const defaultConfig = {
        ...
        serviceName: process.env['SERVICE_NAME'] || 'licensing-verification-platform',
    };
