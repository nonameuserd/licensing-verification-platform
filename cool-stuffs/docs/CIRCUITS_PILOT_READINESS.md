...
- Setup artifacts present: `setup/*.zkey`, `pot*` files, `verification_key.json` — Done (but sensitive)
...
- Ceremony/setup artifacts (`setup/*.zkey`, `pot*.ptau`, `verification_key.json`) — Present but sensitive; treat as secrets
...
- Perf regression gating added — Partially Done (CI writes `artifacts/perf-result.json` and exit-on-regression; threshold override via PERF_THRESHOLD_PERCENT)
...
### Remaining items to be pilot-ready (recommended priority)
...
1. Secure zkey & ceremony artifact management (High)

   - Move final proving keys and any private ceremony artifacts out of the repo into a secured artifact store (recommended: encrypted S3 or vault) with strict ACLs. The current repo contains `setup/*.zkey` and pot files marked "present but sensitive".
...
2. Release governance & environment configuration (High)
...
4. Artifact storage, access controls & CI publishing (High)
...
5. On-chain CI / verifier orchestration (Medium)
...
6. Perf regression automation & baseline management (Medium)
...
7. Security, fuzzing, and third-party audit (Medium)
...
### Short next steps (concrete, actionable)
...
_Document updated: CIRCUITS_PILOT_READINESS.md — reflects implemented CI, signing, on-chain testing, perf gating, and draft release gating._
