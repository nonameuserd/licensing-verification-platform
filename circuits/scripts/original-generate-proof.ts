...
async function main() {
  ...
  try {
    ...
    let derivedPubKey: [string, string];
    let derivedSignature: { S: string; R8: [string, string] };
    {
      ...
      const F = poseidon.F;
      const a = toField(fullInput.examId);
      const b = toField(fullInput.achievementLevel);
      const d = toField(fullInput.issuer);
      const eVal = toField(fullInput.privateKey);
      ...
      let priv: string = fullInput.privateKey as string;
    ...
    try {
      ...
      > = {
        ...
        nullifier: { scalar: String(toField(fullInput.nullifier)) },
      ...
      const serializableFinal = convertSciStrings(finalInput) as Record<
        string,
        unknown
      >;
      ...
      const flatSerializable: Record<string, unknown> = {};
      for (const k of Object.keys(serializableFinal)) {
        const val = (serializableFinal as Record<string, unknown>)[k];
      ...
      logger.error(
        'Failed to build sym-driven canonical input: ' + (e as Error).message
      );
      throw e;
    }
